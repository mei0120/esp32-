...
void csLow() { digitalWrite(TFT_CS, LOW); }
void csHigh() { digitalWrite(TFT_CS, HIGH); }
...
void setup() {
  ...
  delay(1000);
  Serial.println();
  Serial.println("=== ST7789V2 speed test ===");
...
void loop() {
  const uint32_t speeds[] = {1000000, 5000000, 10000000, 20000000, 40000000};
  const uint16_t colors[] = {0xF800, 0x07E0, 0x001F, 0xFFFF, 0xFFE0};
  const char *names[] = {"1MHz red", "5MHz green", "10MHz blue", "20MHz white", "40MHz yellow"};

  for (uint8_t i = 0; i < 5; i++) {
    Serial.println(names[i]);
    tftSpi.setFrequency(speeds[i]);
    fillScreen(colors[i]);
    delay(1800);
  }
}
